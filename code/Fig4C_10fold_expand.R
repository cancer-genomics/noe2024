...
for (fold_name in names(train_folds)) {

    train_IDs<-meth$sample[train_folds[[fold_name]]]
    test_IDs<-meth$sample[-train_folds[[fold_name]]]
    name<- paste0("output/", tolower(fold_name), "_ens.csv")
...
ggsave(here("docs/figure/Fig_4C.pdf"), B)
