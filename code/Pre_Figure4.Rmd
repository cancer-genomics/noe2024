...
author: "Michael Noe, Akshaya Annapragada"
date: "`r Sys.Date()`" 
output:
...
```{r}
outdir <- here("output", "Pre_Figure4.Rmd")
dir.create(outdir)
...
name<-here(outdir, "fold10_ens.csv")
```
...
```{r}
data <- list.files(path=here(outdir),pattern="fold*", full.names = TRUE) %>% 
  lapply(read_csv) %>% 
  bind_rows 

write.csv(data,here(outdir, "All_Scores.csv"))
...
write.csv(healthies,here(outdir, "needtodrop.csv"))
```
