...
t2 <- t %>%
  ...
  summarise(rna_min = (sum(rna_plus) / sum(all)) / rna_plus_length,
            ...
            all = sum(all))
t2l <- reshape2::melt(t2,id.vars = c("mut", "id"))
...
met_is_cases <- ggplot(t2l_sum, aes(y=value, x=factor(variable), fill=mut)) +
  ...
  scale_x_discrete(labels=c("rna_min" = "High\nexpression\nIDH mutant", 
                            "rna_plus" = "High\nexpression\nIDH WT",
                            "met_is_min" = "High\nmethylation\nIDH WT",
                            "met_is_plus" = "High\nmethylation\nIDH mutant")) +
  xlab("Top 1000 differentially methylated or expressed locations\nin isogenic IDH1 mutant or wild-type tumors") + 
  guides(fill=guide_legend(title="IDH status\nin isogenic\nxenografts"),
         colour=guide_legend(title="IDH status\nin isogenic\nxenografts")) 
  
met_is_cases_label <- plot_grid(met_is_cases, labels=c("e"), nrow=1, label_size = 20)
ggsave(here("docs/figure/Fig_3G.jpg"), plot = met_is_cases_label, device = "jpeg", width = 12 , height= 15, units = "in", dpi=300, scale = 0.82)
...
## Figure 3I: cfDNA fragment size distributions and cumulative representations
