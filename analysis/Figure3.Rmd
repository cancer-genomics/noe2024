...
perms  <- permutations(2,6,c(0,1),repeats=TRUE)
keep <- perms[which(rowSums(perms) == 3),] 

#permutation using medians
all <- tibble()
for (j in c(1:nrow(keep))) {
  print(j)
  P <- j
...
saveRDS(all_sum_median,here("data/mouse_permutations_median.rds"))
...
saveRDS(all_sum_mean,here("data/mouse_permutations_mean.rds"))
...
ggsave(here("docs/figure/Fig_3G.jpg"), plot = mouse_plot_label, device = "jpeg", width = 12 , height= 14, units = "in", dpi=600, scale = 0.82)

#met_is_cases <- ggplot(t2l_sum[which(t2l_sum$top == 1000),], aes(y=norm_value, x=factor(variable), fill=mut)) +
