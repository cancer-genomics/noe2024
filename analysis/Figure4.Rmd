...
#ggsave(here("docs/figures/carvalho_comparison_12cycles_ratio.jpg"), plot = plot1, device = "jpeg",width = 20, height=12,units = "in", dpi=300)
...
plot2 <- ggplot(data=new[which((new$method == "regular") & (nchar(as.character(new$motif)) == 6) & (new$label == "healthy unmethylated - pancreatic cancer methylated")) ,], aes(x = group, y=ratio)) +
  ...
  scale_x_discrete(labels=c("CGPLH" = "non-cancer", "CGPLPA" = "pancreatic cancer",
                            ...
                            "CGPLBR" = "breast cancer")) +
  scale_y_continuous(labels = scales::percent)
#ggsave(here("docs/figures/carvalho_comparison_12cycles_extra.jpg"), plot = plot2, device = "jpeg",width = 20, height=12,units = "in", dpi=300)
...
delfi_type<-paste0("DELFI: ",sprintf("%.2f", round(delfi_roc$auc[1],digits = 2))," (",round(delfi_roc$ci[1],2),"-",round(delfi_roc$ci[3],2),")")
...
B <- res %>%
    ...
    scale_y_continuous(expand=c(0, 0.01),
                       ...
                       labels=as.character(
                           c("0", ".25", ".50", ".75", "1.0"))) +
    scale_color_manual(values=c("#77C8DD", "#77dd77", "#d377a0")) +
    theme_classic(base_size=20) +
